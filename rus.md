...
несколькими излучателями и полями гравитации, которые будут притягивать или
...
математик, не физик и даже не разработчик игр, поэтому наверняка есть более
правильные или эффективные реализации того, о чём я буду писать в этой статье.
Несмотря на всё это, это был отличный способ изучить производительность
JavaScript.
...
для этого есть [статьи получше][4]. Если вы всё же решили более глубоко
...
Чтобы получить доступ к объекту `canvas`, нам достаточно получить элемент любым удобным для нас способом.

    var canvas = document.querySelector('canvas');

Canvas может иметь несколько «контекстов», и сейчас нам необходим простой 
2d-контекст, который позволит нам манипулировать холстом как растром 
с помощью набора [стандартных методов][7].

    var ctx = canvas.getContext('2d');

Чтобы максимально увеличить рабочую область для опытов с canvas, мы можем 
задать размеры canvas равными размерам окна браузера.
...
Цикл анимации — это первая непривычная идея, с которой вы столкнётесь, если
...
системы отдельно от её отрисовки, таким образом, вы придёте к двум отдельным
...
гораздо сложнее, если вам потребуется работать с несколькими буферами или
состояниями.
...
Вызов следующей итерации цикла мог бы быть обычным `setTimeout()`, но этот
способ повлечёт выполнение анимации, даже когда она не нужна пользователю
(например, он в другой вкладке), поэтому к нам на помощь приходит
...
котиков на YouTube. Этот метод поддерживается большинством браузеров, но если
...
пропустить их реализацию, чтобы удостовериться, что площадка для
...
представлено в виде двумерного вектора, поэтому начнём с него.

### Вектор

Этот объект будет использоваться **часто**. Если в анимации участвует 10 000
частиц при 30 кадрах в секунду и объект вектора используется для трёх свойств
...
Объект вектора может содержать множество методов, но прямо сейчас нам
...
Теперь, когда мы получили представление вектора, можем создать объект частицы.
Можно задать её свойства или за счёт значений по умолчанию получить частицу, 
расположенную в нуле координат.
...
определённым участкам холста, чтобы не пришлось перерисовывать весь холст.
...
Поле в нашей системе — просто точка, притягивающая или отталкивающая
...
Мои поздравления, вы, наконец, можете посмотреть законченный пример 
на Сopepen.io: [JavaScript Particle System Demo][19]
